...
async function GiveRewards(id: string, difficulty: string) {
    let points: number = 0;
...
async function ScoreSaber(id: string, difficulty: number): Promise<Completion | string | undefined> {
    let completed = false;
    const user = await User.findOne({ userId: id }).exec();
    ...
    if (currentChallenge.type == "playXMaps") {
        ...
        ss.playerScores.forEach((play: any) => {
